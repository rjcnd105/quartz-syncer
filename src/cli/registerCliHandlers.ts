...
const DEFAULT_COMMAND_NAMESPACE = "quartz-syncer";

function namespaceCommand(command: string, namespace: string): string {
	if (command === DEFAULT_COMMAND_NAMESPACE) return namespace;

	return command.startsWith(`${DEFAULT_COMMAND_NAMESPACE}:`)
		? `${namespace}${command.slice(DEFAULT_COMMAND_NAMESPACE.length)}`
		: command;
}

function namespaceCommandMeta(
	meta: CommandMeta,
	namespace: string,
): CommandMeta {
	return {
		...meta,
		name: namespaceCommand(meta.name, namespace),
		examples: meta.examples.map((example) =>
			example.replace(
				`obsidian ${DEFAULT_COMMAND_NAMESPACE}`,
				`obsidian ${namespace}`,
			),
		),
	};
...
export function registerCliHandlers(
	...
	const commandNamespace = plugin.manifest?.id || DEFAULT_COMMAND_NAMESPACE;
	const commandRegistrations = COMMAND_REGISTRY.map((canonical) => ({
		canonical,
		registered: namespaceCommandMeta(canonical, commandNamespace),
	}));
	const handlers: Record<string, CliHandler> = {
		"quartz-syncer": createBaseHandler(
			commandRegistrations.map(({ registered }) => registered),
		),
	...
	const handleCommand = async (
		...
		if (params.flags.has("help")) {
			const meta = commandRegistrations.find(
				({ canonical }) => canonical.name === command,
			)?.registered;
	...
	for (const { canonical, registered } of commandRegistrations) {
		const entry = canonical;
		const registeredEntry = registered;
		const obsidianFlags = buildObsidianFlags(registeredEntry);

		plugin.registerCliHandler(
			registeredEntry.name,
			registeredEntry.description,
			obsidianFlags,
...
function createBaseHandler(commandRegistry: CommandMeta[]): CliHandler {
	return async () => {
