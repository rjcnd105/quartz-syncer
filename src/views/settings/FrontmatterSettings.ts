...
function buildFrontmatterItems(plugin: QuartzSyncer): SettingDefinition[] {
	...
	return [
		...
		},
		{
			...
			desc: "Include only the configured note properties.",
			aliases: ["frontmatter", "keys", "properties"],
			visible: () => !settings.includeAllFrontmatter,
			control: {
				type: "toggle",
				key: "showCustomFrontmatter",
				defaultValue: false,
			},
		},
		{
			name: "Custom property keys",
			desc: "Comma-separated list of note property keys to include.",
			aliases: ["frontmatter", "keys", "properties"],
			visible: () =>
				!settings.includeAllFrontmatter &&
				settings.showCustomFrontmatter,
			control: {
				type: "text",
				key: "customFrontmatterKey",
				defaultValue: "",
