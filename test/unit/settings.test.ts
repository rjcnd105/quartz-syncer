...
describe("settings migration", () => {
	...
	it("preserves custom frontmatter settings during v1 migration", async () => {
		const plugin = createPlugin({
		...
		expect(plugin.settings.showCustomFrontmatter).toBe(true);
		expect(plugin.settings.customFrontmatterKey).toBe("category, rating");
		expect(plugin.settings.settingsSchemaVersion).toBe(4);
	});
